...
router.post('/', authUserMiddleware, async (req, res) => {
  ...
  //res.status(201).json('The entry created successfully');
  res.status(201).json({ message: 'The entry created successfully', status: 'Успшно' });
});
...
router.get("/", authUserMiddleware, async (req, res) => {
  ...
  res.status(200).send({ rents: usercar, cars });
  //res.status(200).send(usercar);
});
